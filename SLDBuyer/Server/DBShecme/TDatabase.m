...
- (TStatement*)cachedStatementForQuery:(NSString*)query {
    return [cachedStatements objectForKey:query];
}

- (void)setCachedStatement:(TStatement*)statement forQuery:(NSString*)query {
...
- (BOOL)goodConnection {
    ...
    TResultSet *rs = [self executeQuery:@"select name from sqlite_master where type='table'"];
...
- (TResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray*)arrayArgs orVAList:(va_list)args {
    ...
    if (!pStmt) {
        do {
            ...
            /*
             当调用 sqlite3_prepare_v2() 函数的时候，是一个将SQL语句编译为sqlite内部一个结构体(sqlite3_stmt pStmt).
             该结构体中包含了将要执行的的SQL语句的信息.
             
             args: Database handle;sql statement utf-8 string;?;sql statement;指向zsql未被使用用的空间地址
             */
    ...
    if (!statement) {
        statement = [[TStatement alloc] init];
    ...
    rs = [TResultSet resultSetWithStatement:statement usingParentDatabase:self];
    [rs setQuery:sql];
...
- (TResultSet *)executeQuery:(NSString*)sql, ... {
    va_list args;
    va_start(args, sql);
...
- (TResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray *)arguments {
...
- (BOOL)executeUpdate:(NSString*)sql error:(NSError**)outErr withArgumentsInArray:(NSArray*)arrayArgs orVAList:(va_list)args {
    ...
    if (shouldCacheStatements && !cachedStmt) {
        cachedStmt = [[TStatement alloc] init];
