...
+ (id)resultSetWithStatement:(TStatement *)statement usingParentDatabase:(TDatabase*)aDB {
    
    TResultSet *rs = [[TResultSet alloc] init];
...
- (void)setParentDB:(TDatabase *)newDb {
    parentDB = newDb;
}
...
- (void)setStatement:(TStatement *)value {
    if (statement != value) {
        [statement release];
